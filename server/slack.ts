...
/** 최근 윈도우 내 '나쁜' 점검의 종류별 집계 — 메시지를 '접속 실패'와 '속도지연'으로 구분해 표시하기 위함. */
type BadBreakdown = {
...
function buildAlarm(
  ...
  return {
    title: `🔴 ${p.label}`,
    detail: `${p.url} · 최근 ${windowSize}회 중 ${parts.join(' · ')}${extra}`,
  };
}
...
async function fire(
  ...
  now: number,
  windowSize: number,
  bd: BadBreakdown, // 실패/지연 종류별 집계 — 메시지 구분용
  ...
  const total = bd.fails + bd.slows; // 누적(최근 N회 중 총 실패/지연) — 이력 badCount 로 저장
  const { title, detail } = buildAlarm(p, windowSize, bd, fresh, renotify);
...
export async function evaluateAndNotify(): Promise<void> {
  ...
  for (const p of listPages()) {
    ...
    if (recent.length < slack.windowSize) continue; // 데이터가 N회 미만이면 평가 보류
    const badChecks = recent.filter(c => isBad(c, criticalMs));
    // 같은 '나쁨'이라도 응답 자체 실패(접속 실패)와 느림(속도지연)을 나눠 집계 → 메시지에서 구분.
    const slowChecks = badChecks.filter(c => c.ok); // ok=1 이면서 isBad → 심각(느림)
    const bd: BadBreakdown = {
      fails: badChecks.filter(c => !c.ok).length, // ok=0 → 응답 자체 실패
    ...
    if (!firing) {
      ...
      if (fresh >= slack.threshold && cooldownPassed) {
        await fire(slack, p, now, slack.windowSize, bd, fresh, maxId, false);
      }
    } else {
      ...
      if (recentOk) {
        ...
        await fire(slack, p, now, slack.windowSize, bd, fresh, maxId, true);
      }
      // fresh < threshold → 침묵
    }
  }
